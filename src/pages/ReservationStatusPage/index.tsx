...
export function ReservationStatusPage() {
	...
	const getRoomName = (roomId: string) =>
		rooms.find((r: { id: string; name: string }) => r.id === roomId)?.name ??
		roomId;
		...
		<div css={css`background: ${colors.white}; padding-bottom: 40px;`}>
			...
			<div css={css`padding: 0 24px;`}>
				...
				>
					{/* 시간 헤더 */}
					<div
						css={css`display: flex; align-items: flex-end; margin-bottom: 8px;`}
					>
					...
					{rooms.map((room: { id: string; name: string }, index: number) => {
						...
						return (
							...
							>
								<div
									css={css`width: 80px; flex-shrink: 0; padding-right: 8px;`}
								>
									<Text
										typography="t7"
										fontWeight="medium"
								...
								<div
									css={css`flex: 1; height: 24px; background: ${colors.white}; border-radius: 6px; position: relative; overflow: visible;`}
								>
									{roomReservations.map(
										(res: {
											...
											const left =
												(timeToMinutes(res.start) / TOTAL_MINUTES) * 100;
											const width =
												((timeToMinutes(res.end) - timeToMinutes(res.start)) /
													TOTAL_MINUTES) *
												100;
											...
											return (
												...
												>
													<div
														...
														onClick={() =>
															setActiveReservation(isActive ? null : res.id)
														}
														css={css`
            ...
            `}
					>
						<Text
							...
							fontWeight="medium"
							color={
								message.type === "success" ? colors.blue600 : colors.red500
							}
						>
							{message.text}
						</Text>
			...
			<div css={css`padding: 0 24px;`}>
				...
				{myReservationList.length === 0 ? (
					<div
						css={css`padding: 40px 0; text-align: center; background: ${colors.grey50}; border-radius: 14px;`}
					>
